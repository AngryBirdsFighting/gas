		...
		<div class="filter-container">
			<el-form :inline="true" :model="listQuery" class="demo-form-inline">
				...
				<el-form-item label="统计时间">
					<date-limit-picker ref="dateLimitPicker"></date-limit-picker>
				</el-form-item>
				<el-form-item>
...
import DateLimitPicker from "../../components/DateLimitPicker"; //日期组件
...
export default {
    components: {
        ...
        "date-limit-picker": DateLimitPicker, //日期组件
    ...
    data() {
        return {
            ...
            pickerBeginDateBefore: {
                disabledDate: time => {
                    let beginDateVal = this.listQuery.toTime;
                    if (beginDateVal) {
            ...
            pickerBeginDateAfter: {
                disabledDate: time => {
                    let a = time;
                    let beginDateVal = this.listQuery.fromTime;
    ...
    watch: {
        isOpened(newVal, oldVal) {
            window.setTimeout(() => {
                ...
                document.getElementById("echartBox").style.height = $(".el-table").height() + "px"; //折线图的高度
                document.getElementById("mileDayLine").style.width = $(".el-table").width() - 40 + "px"; //折线图的高度
                $("#mileDayLine div:first-child").width(
                    $("#mileDayLine").width()
                );
                if(this.mileDayLine && this.checkTrailEchartVisible){
                    this.mileDayLine.resize();
                }
            }, 300);
        }
    },
    ...
    methods: {
        ...
        getCarId() {
            ...
            if (this.departData.checkBox) {
                if (
                    ...
                    this.listQuery.carId.indexOf(",") > -1
                ) {
        ...
        queryFlag() {
            const vm = this;
            let flag = true;
            const dateTime = vm.$refs.dateLimitPicker.datePicker; //父组件获取子组件数据this.$refs.第一个datePicker是父组件ref值，第二个是子组件model值
            if (dateTime) {
                vm.listQuery.fromTime = dateTime.fromTime;
                vm.listQuery.toTime = dateTime.toTime;
            }
            ...
            vm.listQuery.fromTime = utils.turnTime(vm.listQuery.fromTime);
            vm.listQuery.toTime = utils.turnTime(vm.listQuery.toTime);
            return flag;
        },
                    ...
                    if (res.status == 200) {
                        if(res.data && res.data.length>0){
                            ...
                            if (vm.mileDayLine) {
                                ...
                                vm.mileDayLine.clear(); //清空echarts画布
                            }
                        }
        ...
        exportFormLists() {
            ...
            utils.exportLists(
                this.listQuery,
                "/proxy/report/mileage/exportMileage"
            );
        },
        ...
        handelData(type) {
            ...
            document.getElementById("echartBox").style.width = $(".el-table").width() + 2 + "px"; //折线图的高度
            document.getElementById("echartBox").style.height = $(".el-table").height() + "px"; //折线图的高度
            $("#mileDayLine").height($("#echartBox").height());
            if (vm.mileDayLine) {
                vm.mileDayLine.clear(); //清空echarts画布
            }
