...
<template>
	<div>
		...
		<div v-show="checkTrailEchartVisible" class="box" id="echartBox5">
			<div class="el-icon-circle-close close" @click="closeEchart"></div>
			<div class="mileDayLine" id="alarmMonthLine"></div>
		</div>
	</div>
</template>
	...
	export default {
		...
		watch: {
			isOpened(newVal, oldVal) {
				window.setTimeout(() => {
					document.getElementById("echartBox5").style.width = $('.el-table').width() + 2 + 'px';//折线图的高度
					document.getElementById("echartBox5").style.height = $('.el-table').height() + 'px';//折线图的高度
					document.getElementById("alarmMonthLine").style.width = $('.el-table').width() - 40 + 'px';//折线图的高度
					$('#alarmMonthLine div:first-child').width($('#alarmMonthLine').width());
					if(this.alarmMonthLine && this.checkTrailEchartVisible){
						this.alarmMonthLine.resize();
					}
				}, 300)
			}
		},
		...
		methods: {
		          ...
		          if(res.status == 200){
						...
						if(res.data && res.data.length > 0){
							...
							if(type == "bar" || vm.checkTrailEchartVisible){
			          		...
			          		vm.list = [];
			          		if (vm.alarmMonthLine) {
			          			vm.checkTrailEchartVisible = false;
				                vm.alarmMonthLine.clear(); //清空echarts画布
				            }
			          	}
		            }else{
			...
			handelData(data){
				...
				document.getElementById("echartBox5").style.width = $('.el-table').eq(1).width() + 2 + 'px';//折线图的高度
				document.getElementById("echartBox5").style.height = $('.el-table').eq(1).height() + 'px';//折线图的高度
				$('#alarmMonthLine').height( $('#echartBox5').height() );
			...
			drawLine(){
				...
				initEcharts().then(echarts => {
					vm.alarmMonthLine = echarts.init(document.getElementById('alarmMonthLine'));
					// 绘制折线图
			        vm.alarmMonthLine.setOption({
			            tooltip: {
			              trigger: 'axis',
			              axisPointer: {
			                  type: 'cross',
